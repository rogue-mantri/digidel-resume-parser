...
app = FastAPI(title="DIGIDEL Hiring Agent", description="Automated resume parsing, job matching, and first-pass filtering for the DIGIDEL hiring ecosystem.", version="2.0.0")
...
@app.get("/")
def read_root():
    return {"status": "running", "service": "DIGIDEL Hiring Agent", "version": "2.0.0", "crm_mode": os.environ.get("CRM_MODE", "standalone"), "endpoints": {"health": "/health", "crm_health": "/crm/health", "jobs": "/crm/jobs (GET)", "job_match": "/match (POST)", "parse": "/parse (POST, upload file)", "pipeline": "/pipeline (POST, upload file)", "batch": "/batch (POST, upload multiple files)", "stats": "/stats", "results": "/results", "dashboard": "/dashboard (HTML UI)"}}
...
def parse_resume(file: UploadFile = File(...)):
    ...
    try:
        ...
        return {"success": True, "file_name": file.filename, "file_size": extraction['file_size'], "format": extraction['format'], "text_length": len(extraction['text']), "profile": profile}
    finally:
        Path(tmp_path).unlink(missing_ok=True)

@app.post("/pipeline")
def full_pipeline(file: UploadFile = File(...), role: str = Form("react_developer"), job_id: Optional[str] = Form(None), candidate_name: Optional[str] = Form(None), candidate_email: Optional[str] = Form(None)):
    allowed_extensions = {'.pdf', '.docx', '.doc', '.txt', '.rtf'}
    ext = Path(file.filename).suffix.lower()
    if ext not in allowed_extensions:
        raise HTTPException(400, f"Unsupported file type: {ext}")
    ...
    try:
        ...
        result = {"success": True, "file_name": file.filename, "format": extraction['format'], "file_size": extraction['file_size'], "text_length": len(extraction['text']), "parse_confidence": profile.get('parse_confidence', 0), "profile": {"name": profile.get('full_name'), "email": profile.get('email'), "phone": profile.get('phone'), "location": profile.get('location'), "experience_years": profile.get('years_experience'), "current_title": profile.get('current_title'), "skills": profile.get('skills', {}), "skills_count": profile.get('skills_count', 0), "education": profile.get('education', {}), "keywords": profile.get('keywords', []), "links": profile.get('links', {})}, "filter": {"role": role, "decision": filter_result['decision'], "confidence": filter_result['confidence'], "summary": filter_result['summary'], "failed_rules": filter_result['failed_rules'], "yellow_flags": filter_result['yellow_flags']}, "match": match_result, "processed_at": datetime.now().isoformat()}
        processed_resumes.append(result)
        session_stats['total_processed'] += 1
        if filter_result['decision'] == 'PASS': session_stats['passed'] += 1
        elif filter_result['decision'] == 'REJECT': session_stats['rejected'] += 1
        else: session_stats['yellow_flags'] += 1
        return result
    finally:
        Path(tmp_path).unlink(missing_ok=True)
    ...
    for file in files:
        ...
        try:
            ...
            if target_job:
                ...
                if matches: match_result = matches[0]
            result = {"file_name": file.filename, "success": True, "format": extraction['format'], "profile": {"name": profile.get('full_name'), "email": profile.get('email'), "experience_years": profile.get('years_experience'), "skills_count": profile.get('skills_count', 0)}, "filter": {"decision": filter_result['decision'], "confidence": filter_result['confidence'], "summary": filter_result['summary']}, "match": match_result, "processed_at": datetime.now().isoformat()}
...
def create_job_standalone(job_id: str = Form(...), title: str = Form(...), description: str = Form(...), required_skills: str = Form(...), min_experience: float = Form(0), max_experience: float = Form(99), department: Optional[str] = Form(None), location: Optional[str] = Form(None), salary_min: Optional[float] = Form(None), salary_max: Optional[float] = Form(None)):
    job = {"name": job_id, "job_title": title, "description": description, "required_skills": [s.strip() for s in required_skills.split(",")], "min_experience": min_experience, "max_experience": max_experience, "department": department, "location": location, "salary_range": {"min": salary_min, "max": salary_max} if salary_min and salary_max else None, "source": "standalone", "created_at": datetime.now().isoformat()}
    job_descriptions[job_id] = job
...
@app.get("/stats")
def get_stats():
    return {"session_stats": session_stats, "total_in_session": len(processed_resumes), "pass_rate": round(session_stats['passed'] / max(session_stats['total_processed'], 1) * 100, 2), "cached_jobs": len(job_descriptions)}
